...
#define YY___PRESERVE_NONE 22
#define YY___BUILTIN 23
#define YY___PURE 24
#define YY___CONST 25
#define YY__STAR 26
#define YY_LABEL 27
#define YY__SLASH 28
#define YY_BYVAL 29
#define YY_BOOL 30
#define YY_UINT8_T 31
#define YY_UINT16_T 32
#define YY_UINT32_T 33
#define YY_UINT64_T 34
#define YY_UINTPTR_T 35
#define YY_CHAR 36
#define YY_INT8_T 37
#define YY_INT16_T 38
#define YY_INT32_T 39
...
_yy_state_start:
	...
	switch (ch) {
		...
		case 'B':
			ch = *++YYPOS;
			if (ch != 'y') goto _yy_tunnel_5;
			ch = *++YYPOS;
			if (ch != 'V') goto _yy_tunnel_5;
			ch = *++YYPOS;
			if (ch != 'a') goto _yy_tunnel_5;
			ch = *++YYPOS;
...
static int parse_ir(int sym, ir_loader *loader) {
	...
	const char *str;
	size_t len;
	if (YY_IN_SET(sym, (YY_EXTERN,YY_STATIC,YY_VAR,YY_CONST,YY_FUNC), "\012\003\001\000\000\000\000\000")) {
		do {
			if (sym == YY_EXTERN) {
				sym = get_sym();
				flags = 0;
...
_yy_state_13:
		...
		} while (YY_IN_SET(sym, (YY_EXTERN,YY_STATIC,YY_VAR,YY_CONST,YY_FUNC), "\012\003\001\000\000\000\000\000"));
	} else if (sym == YY__LBRACE) {
...
static int parse_ir_modifier(int sym, ir_parser_ctx *p) {
	ir_val size, align;
	if (sym != YY_BYVAL) {
		yy_error_sym("'ByVal' expected, got", sym);
	}
	sym = get_sym();
	if (sym != YY__LPAREN) {
		yy_error_sym("'(' expected, got", sym);
	...
	sym = get_sym();
	sym = parse_const(sym, IR_I32, &size);
	if (sym != YY__COMMA) {
		yy_error_sym("',' expected, got", sym);
	}
	sym = get_sym();
	sym = parse_const(sym, IR_I32, &align);
	if (sym != YY__RPAREN) {
		yy_error_sym("')' expected, got", sym);
	}
	sym = get_sym();
